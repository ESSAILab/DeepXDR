...
const ReadTextFileArgsSchema = z.object({
  ...
  limit: z.number().optional().describe('Maximum number of lines to return when not using head/tail. Default is 2000')
});
...
const readTextFileHandler = async (args: z.infer<typeof ReadTextFileArgsSchema>) => {
  ...
  if (args.tail) {
    ...
    // 默认使用 limit 参数，默认值为 2000
    const lineLimit = args.limit ?? 2000;
    content = await readFileContentWithLimit(validPath, lineLimit);
  }
...
server.registerTool(
  "read_text_file",
  {
    ...
    description:
      "Read the contents of a file from the file system as text. " +
      "Handles various text encodings and provides detailed error messages " +
      "if the file cannot be read. Use this tool when you need to examine " +
      "the contents of a single file. Use the 'head' parameter to read only " +
      "the first N lines of a file, or the 'tail' parameter to read only " +
      "the last N lines of a file. If neither head nor tail is specified, " +
      "returns up to 2000 lines by default (adjustable via 'limit' parameter). " +
      "Each line is truncated to 2000 characters max with ' ... [truncated]' appended if exceeded. " +
