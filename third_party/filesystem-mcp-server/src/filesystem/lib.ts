...
/**
 * 截断单行内容，如果超过最大长度则添加截断标记
 */
export function truncateLine(line: string, maxLength: number = MAX_LINE_LENGTH): string {
  if (line.length <= maxLength) {
    return line;
  }
  return line.substring(0, maxLength) + TRUNCATED_MARKER;
}

/**
 * 对多行内容的每一行应用截断处理
 */
export function truncateLines(content: string, maxLength: number = MAX_LINE_LENGTH): string {
  return content.split('\n').map(line => truncateLine(line, maxLength)).join('\n');
}
...
/**
 * 带行数限制的文件读取函数，默认最多读取指定行数，每行超长会被截断
 */
export async function readFileContentWithLimit(
  filePath: string,
  lineLimit: number = 2000,
  charLimit: number = MAX_LINE_LENGTH
): Promise<string> {
  const fileHandle = await fs.open(filePath, 'r');
  try {
    const lines: string[] = [];
    ...
    // 读取 chunks 直到收集到足够的行数或到达文件末尾
    while (lines.length < lineLimit) {
      const result = await fileHandle.read(chunk, 0, chunk.length, bytesRead);
      if (result.bytesRead === 0) break; // 文件结束
      bytesRead += result.bytesRead;
      buffer += chunk.slice(0, result.bytesRead).toString('utf-8');
      ...
      if (newLineIndex !== -1) {
        ...
        for (const line of completeLines) {
          // 对每一行应用截断
          lines.push(truncateLine(line, charLimit));
    ...
    // 如果还有剩余内容且还需要行数，添加它
    if (buffer.length > 0 && lines.length < lineLimit) {
      lines.push(truncateLine(buffer, charLimit));
    }
