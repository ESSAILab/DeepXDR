...
Agent Security Analysis Mode uses an independent pipeline:

```mermaid
flowchart LR
    Shell["User shell / AI agent"] --> Shim["scripts/nono PATH shim"]
    Shim --> RealNono["Real nono"]
    ...
    Guard --> Risk["Change risk analysis"]
    Risk --> PGAgent["PostgreSQL agent sessions"]
    PGAgent --> APIAgent["FastAPI"]
    APIAgent --> AgentUI["Web UI Agent Security Analysis Mode"]
    AgentUI --> Accept["Accept change"]
    AgentUI --> Rollback["Execute rollback"]
    Rollback --> Restore["nono rollback restore"]
```

Agent pipeline flow:
1. After putting this repository's `scripts/` directory before the real nono binary in `PATH`, users still run `nono` as usual, but the command enters the DeepXDR PATH shim first.
2. The shim calls the real nono and extracts the rollback session diff after the command exits.
3. The diff is written to MinIO/S3. The event carries only metadata such as diff URI, original request, run id, and nono session id.
4. `baseline_adjudication` routes the agent session event to Kafka `agent.session.finished`.
5. AgentGuard consumes the session event, selects a full-diff, per-file-summary, or clipped-summary strategy according to diff size, and generates Chinese change risk analysis.
6. The Web UI displays the alert in Agent Security Analysis Mode, where users can accept the change, execute rollback, or delete the alert.
...
`docker-compose-app.yml` configuration notes:
...
services:
  ...
  filebeat:
    ...
    user: root
    volumes:
      # [Required] Collect the three telemetry-source data types through shared volumes: the `cms-shared` volume for OpenRASP logs, the `falco-logs-volume` volume for Falco logs, and the host `/var/log/suricata` directory for Suricata logs. The specific path names must match the volumes defined by the three telemetry sources in docker-compose.yml.
...
`docker-compose-agent.yml` configuration notes:
...
```yaml
  web-ui:
    ...
    environment:
      ...
      # [Required] Same as the ai-agent configuration above. The Web UI uses this value when calling backend APIs.
      BACKEND_API_KEY: <your-random-token>
    networks:
      - security-net
    ports:
...
#### 7.3 Route shell nono commands through the DeepXDR shim
...
The four samples cover:

| Scenario | Description |
| --- | --- |
| `small` | Small diff. Uses the complete file diff for one-pass change risk analysis. |
| `medium` | Multi-file medium diff. Generates per-file change summaries first, then performs aggregate analysis. |
...
Switch to "Agent Security Analysis Mode" in the upper-right corner. This mode shows pending manual reviews, completed rollbacks, total alerts, and each agent alert's risk level, status, summary, change risk analysis, changed files, and diff preview.

Available actions:

| Action | Effect |
| --- | --- |
| Accept change | Marks the alert as accepted without rolling back code. |
...
- `BACKEND_API_KEY` must match between ai-agent and web-ui. Use a long random string in production.
- Inject `OPENAI_API_KEY`, object-storage credentials, and database passwords through environment variables, CI secrets, or the deployment platform's secret manager.
- Web UI Agent Security Analysis Mode is intended for human review of high-risk agent changes. Do not execute rollback automatically without human confirmation and audit.
...
| Agent security analysis enhancement | Build on the current nono integration and extend auditing for more AI-agent runtimes, tool calls, network access, and execution traces. |
